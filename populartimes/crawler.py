	...
	for day in popularity:
		...
		if pop_times is not None:
			for hour_info in pop_times:
				...
				hour = hour_info[0]
				pop_json[day_no - 1][hour] = {
					'relative_bar': hour_info[1],
					'text': hour_info[2]
				}
...
def get_data_from_search(place_identifier, proxy_host=False):
	...
	#search_url = 'http://httpbin.org/ip'
	logging.info("searchterm: " + search_url)

	req = requests.Session()
	if proxy_host:
		 req.proxies = proxy_host
	...
	to_return['rating'] = index_get(info, 4, 7)
	to_return['rating_n'] = index_get(info, 4, 8)

	to_return['latitude'] = index_get(info, 9, 2)
	to_return['longitude'] = index_get(info, 9, 3)
	...
	if reviews is not None:
		for review in reviews:
			r = {}
			r['time'] = index_get(review, 1)
			r['review_text'] = index_get(review, 3)
			r['map_location'] = index_get(review, 5)

			to_return_reviews.append(r)

	to_return['reviews'] = to_return_reviews

	to_return['popular_times'] = index_get(info, 84, 0)


	# current_popularity is also not available if popular_times isn't
	to_return['current_popularity'] = index_get(info, 84, 7, 1)
